...
from apps import my_style
from apps import table_to_cyto
...
# 上段右　処理結果の出力用ボタン
        html.Div(
            ...
            children=[
                html.Div([
                    html.Button('最短経路', 
                        className="app-button", id='id1_btn_done', n_clicks=0,
                    ),
                ], style={'height':'80px','margin-top':'30px'}),
                html.Div([
                    html.Button('事例読込', 
                        className="app-button", id='id1_btn_load', n_clicks=0,
                    ),
                ], style={'height':'80px'}),
                html.Div([
                    html.Button('ク リ ア', 
                        className="app-button", id='id1_btn_clear', n_clicks=0,
                    ),
                ], style={'height':'80px'}),
...
@app.callback(
    Output('id1_problem', 'elements'),
    [Input('id1_table', 'data')]
)
...
@app.callback(
    Output('id1_answer', 'elements'),
    [Input('id1_btn_done', 'n_clicks'),
     Input('id1_btn_load', 'n_clicks'),
     Input('id1_btn_clear', 'n_clicks')],
    [State('id1_table', 'data')]
)
def update_answer(btn_done, btn_load, btn_clear, data):
    ...
    if button_id == 'id1_btn_done':
        return table_to_cyto.get_shortest_path(data)
    elif button_id == 'id1_btn_load':
        return []
    elif button_id == 'id1_btn_clear':
        return []

@app.callback(
    Output('id1_table', 'data'),
    [Input('id1_btn_load', 'n_clicks'),
     Input('id1_btn_clear', 'n_clicks')],
)
